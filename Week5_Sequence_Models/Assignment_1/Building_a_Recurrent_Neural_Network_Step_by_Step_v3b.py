...
def rnn_cell_backward(da_next, cache):
    ...
    dz = 1-np.tanh(np.dot(Wax,xt)+np.dot(Waa,a_prev)+ba)**2

    # compute the gradient of the loss with respect to Wax (≈2 lines)
    dxt = np.dot(Wax.transpose(),da_next*dz)
    dWax = np.dot(da_next*dz,xt.transpose())

    # compute the gradient with respect to Waa (≈2 lines)
    da_prev = np.dot(Waa.transpose(),da_next*dz)
    dWaa = np.dot(da_next*dz,a_prev.transpose())
...
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    dWf = np.dot(dft,np.concatenate((a_prev,xt), axis=0).transpose())
    dWi = np.dot(dit,np.concatenate((a_prev,xt), axis=0).transpose())
    dWc = np.dot(dcct,np.concatenate((a_prev,xt), axis=0).transpose())
    dWo = np.dot(dot,np.concatenate((a_prev,xt), axis=0).transpose())
    ...
    da_prev = np.dot(parameters_tmp['Wf'][:,:n_a].transpose(),dft)+np.dot(parameters_tmp['Wi'][:,:n_a].transpose(),dit)+np.dot(parameters_tmp['Wc'][:,:n_a].transpose(),dcct)+np.dot(parameters_tmp['Wo'][:,:n_a].transpose(),dot)
    dc_prev = dc_next*ft+ot*(1-np.tanh(c_next)**2)*ft*da_next
    dxt = np.dot(parameters_tmp['Wf'][:,n_a:].transpose(),dft)+np.dot(parameters_tmp['Wi'][:,n_a:].transpose(),dit)+np.dot(parameters_tmp['Wc'][:,n_a:].transpose(),dcct)+np.dot(parameters_tmp['Wo'][:,n_a:].transpose(),dot)
    ### END CODE HERE ###
